...
public class ComponentAutoBindToolInspector : Editor
{
    ...
    private void GenAutoBindCode()
    {
        ...
        string searchPath = !string.IsNullOrEmpty(m_Target.CodePath) ? m_Target.CodePath : m_Setting.CodePath;
        string codePath = string.Empty;
        
        // 获取代码路径
        var guids = AssetDatabase.FindAssets(className, new string[] {"Assets/Scripts/Compiled"});
        if (guids.Length == 0)
        {
            Debug.LogError($"获取脚本 => {className} 不存在");
            return;
        }
        ...
        using (StreamWriter sw = new StreamWriter($"{codePath}/{className}.BindComponents.cs"))
        {
            ...
            for (int i = 0; i < m_Target.BindDatas.Count; i++)
            {
                ...
                sw.WriteLine($"\t\t\t{filedName} = ({data.BindCom.GetType().Name})autoBindTool.GetBindComponent({i});");
            }
